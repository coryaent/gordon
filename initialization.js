...
const garageEvents = new EventEmitter()
  ...
  .once("allNodes", async (garageNodes) => {
    ...
    for (let node of garageNodes) {
      ...
      if (swarmNode.Spec.Labels[process.env.GORDON_CAPACITY_LABEL] != "null") {
        capacity = Number.parseInt(
          swarmNode.Spec.Labels[process.env.GORDON_CAPACITY_LABEL],
        );
      }
      ...
      if (swarmNode.Spec.Labels[process.env.GORDON_TAGS_LABEL]) {
        tags = JSON.parse(
          swarmNode.Spec.Labels[process.env.GORDON_TAGS_LABEL],
        ).split(",");
      }
    ...
    try {
      var response = await fetch(
        `http://${process.env.GORDON_ADMIN_ENDPOINT}/v1/layout`,
        {
          method: "POST",
          headers: headers,
  ...
  .once("allChanges", async () => {
    ...
    if (response.status == 200) {
      ...
      try {
        var application = await fetch(
          `http://${process.env.GORDON_ADMIN_ENDPOINT}/v1/layout/apply`,
          {
            method: "POST",
            headers: headers,
            body: JSON.stringify({
...
let knownNodesWatch = setInterval(async () => {
  ...
  try {
    var response = await fetch(
      `http://${process.env.GORDON_ADMIN_ENDPOINT}/v1/status`,
      {
        headers: headers,
      },
    );
  ...
  if (response.status == 200) {
    ...
    if (upNodes > foundNodes) {
      console.log(
        `found ${upNodes}/${process.env.GORDON_EXPECTED_NODE_COUNT} garage nodes`,
      );
      foundNodes = upNodes;
    }
...
let changesWatch = setInterval(async () => {
  ...
  if (response.status == 200) {
    ...
    if (numRoles == Number.parseInt(process.env.GORDON_EXPECTED_NODE_COUNT)) {
      console.log(
        `found ${numRoles}/${process.env.GORDON_EXPECTED_NODE_COUNT} roles`,
      );
    ...
    console.log(
      `found ${numChanges}/${process.env.GORDON_EXPECTED_NODE_COUNT} staged changes`,
    );
