...
async function create(bucketName) {
  ...
  try {
    ...
    let response = await fetch(
      `http://${process.env.GORDON_ADMIN_ENDPOINT}/v1/bucket`,
      {
        method: "POST",
        headers: headers,
        body: JSON.stringify({
    ...
    if (!process.env.GARAGE_ACCESS_KEY_ID) {
      response = await fetch(
        `http://${process.env.GORDON_ADMIN_ENDPOINT}/v1/key`,
        {
          method: "POST",
          headers: headers,
          body: JSON.stringify({
    ...
    response = await fetch(
      `http://${process.env.GORDON_ADMIN_ENDPOINT}/v1/bucket/allow`,
      {
        method: "POST",
        headers: headers,
        body: JSON.stringify({
    ...
    console.error(
      ...
      process.env.GORDON_NEW_BUCKET_NAME,
    );
    process.exit(1);
  }
